...
2. **見通し判定アルゴリズム（旧・東向きベランダ: `is_east_view_favorable`）**:
    ```rust
...
### 4.3 2026年9月15日: アンテナ北向き移設に伴う見通し判定の刷新

2026年9月15日、アンテナ設置場所を「東向きベランダ」から「北向きベランダ」へと移設しました。これにより、見通し幾何学と判定アルゴリズムは以下のようにアップデートされます：

```text
【新・マンション北向きベランダの見通し幾何学】
...
1. **新・見通し開口角（北半球視野: $270^\circ \sim 360^\circ / 0^\circ \sim 90^\circ$）**:
   - アンテナ前面が北方向に開けており、真西（$270^\circ$）〜真北（$0^\circ/360^\circ$）〜真東（$90^\circ$）の全空が開口。
   - ISS（軌道傾斜角 $51.6^\circ$）が日本上空を南西から北東へ抜けるパスにおいて、天頂付近から北東方面（東北・太平洋北部上空）へ遠ざかる後半フェーズ、あるいは北西から進入するパスを直達波で安定して追尾できます。
2. **新・見通し判定アルゴリズム（北向き: `is_north_view_favorable`）**:
   ```rust
   pub fn is_north_view_favorable(&self) -> bool {
       let norm = self.peak_azimuth_deg.rem_euclid(360.0);
       norm >= 270.0 || norm <= 90.0
   }
   ```
3. **ISS APRS / SSTV 受信戦略の更新**:
   - ピーク仰角が $50^\circ$ 以上の天頂パス（Overhead Pass）はベランダ庇（ひさし）を抜けて直達波を受信可能。
   - ピーク方位が北東（NE: 約 $45^\circ$）や北西（NW: 約 $315^\circ$）のパスは、中・低仰角であっても建物に遮られず良好な S/N 比で AX.25 パケットを復調可能です。
