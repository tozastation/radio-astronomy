...
## 6. 実観測データ（仰角 72.1° パス）のファーストライト解析と DSP 高度化

2026年9月15日、仰角 72.1°（天頂付近・極めて良好な通過パス）において、XW-2A の生IQデータ（`raw.u8`）の保全と Discord 自動通知が初めて成功しました。本節では、得られた初期スペクトログラム画像の物理的解読結果と、それを踏まえて実施した DSP パイプラインの高度化（コヒーレントデシメーション、適応スケルチ、軟化キーイング）の数理モデルを詳解します。

### 6.1 スペクトログラム初期画像（ファーストライト）の物理的読解

初回観測のスペクトログラムには、**「宇宙からの本物の CW 電波」** と **「初期 DSP の物理的振る舞い」** が明確に記録されていました：

1. **下部の常時発光する水平線（約 750 Hz）**:
   - 観測時間（約 5 分間）の全区間にわたり、下から約 13.6%（$750\,\text{Hz} / 5512.5\,\text{Hz}$）の周波数に強烈な連続線が描画されていました。
   - これは、受信帯域幅 $240\,\text{kHz}$ 全体の広帯域熱雑音を未フィルタのまま包絡線検波したことにより、正の直流バイアス（レイリー分布の平均値）が発生し、$750\,\text{Hz}$ の BFO 正弦波が常時励起されたためです。
2. **右上の断続的なパルス群（本命の XW-2A モールス信号！）**:
   - 通過後半（衛星が天頂を過ぎ、アンテナ利得および偏波面が好条件となった時間帯）の上部帯域に、「トン・ツー」と美しく途切れる水平なパルス列が鮮明に出現しました。
   - これは XW-2A が送信するコールサイン・テレメトリのキーイング電波そのものです。上部に等間隔で複数段現れていたのは、$750\,\text{Hz}$ トーンが急峻なパルスで変調・クランプされたことによる **奇数次高調波（3次=2250Hz, 5次=3750Hz, 7次=5250Hz の歪み・キークリック成分）** です。

---

### 6.2 コヒーレント積分（検波前複素平均）vs インコヒーレント積分（検波後平均）の数理

下部の常時発振を解消し、ノイズフロアを劇的に低下させるため、検波処理を「検波後平均（Incoherent）」から「検波前複素デシメーション（Coherent）」へ刷新しました。

#### ① 記号一覧
| 記号 | 物理的・工学的意味 | 単位 / 代表値 |
| :--- | :--- | :--- |
| $s[k]$ | SDR から入力される複素ベースバンド信号 ($I[k] + j Q[k]$) | 無次元（正規化値） |
| $n_I[k], n_Q[k]$ | ガウス白色雑音成分（平均 0、分散 $\sigma_n^2$） | 無次元 |
| $M$ | デシメーション比（$240\,\text{kSPS} / 11.025\,\text{kSPS}$） | $M \approx 21.768$ |
| $\bar{I}, \bar{Q}$ | ブロック内のコヒーレント平均同相・直交成分 | 無次元 |
| $r_{\text{incoherent}}$ | 検波後平均による包絡線振幅 | レイリー分布の期待値 $\approx 1.253 \sigma_n$ |
| $r_{\text{coherent}}$ | 検波前複素積分による包絡線振幅 | 無信号時期待値 $\approx 1.253 \frac{\sigma_n}{\sqrt{M}}$ |

#### ② 日本語での読み解き
各サンプルの振幅 $\sqrt{I^2 + Q^2}$ を計算してから時間平均（インコヒーレント積分）しても、雑音の絶対値は常に正の値（レイリー分布）をとるため、平均値 $\sigma_n \sqrt{\pi/2}$ はどれだけ時間をかけてもゼロになりません。
一方、振幅を取る前に複素平面上の直交座標 $(I, Q)$ のまま足し合わせる（コヒーレント積分）と、平均 0 のガウス雑音は正負で互いに打ち消し合います。これにより、デシメーション区間 $M$ サンプルの積算で雑音分散は直接 $1/M$（約 $-13.4\,\text{dB}$）に激減します。

#### ③ 展開ステップと直感イメージ
1. **検波前複素平均の統計量**:
   $$\bar{I} = \frac{1}{M} \sum_{k=0}^{M-1} I[k] \sim \mathcal{N}\left( s_I, \frac{\sigma_n^2}{M} \right), \quad \bar{Q} = \frac{1}{M} \sum_{k=0}^{M-1} Q[k] \sim \mathcal{N}\left( s_Q, \frac{\sigma_n^2}{M} \right)$$
2. **無信号時（$s_I = s_Q = 0$）の振幅期待値比**:
   $$\frac{\mathbb{E}[r_{\text{coherent}}]}{\mathbb{E}[r_{\text{incoherent}}]} = \frac{\frac{\sigma_n}{\sqrt{M}} \sqrt{\frac{\pi}{2}}}{\sigma_n \sqrt{\frac{\pi}{2}}} = \frac{1}{\sqrt{M}} \approx \frac{1}{\sqrt{21.768}} \approx 0.214 \quad (-13.4\,\text{dB})$$
3. これにより、検波器に入力される前段階で広帯域雑音電力の 95.4% が除去されます。

---

### 6.3 動的ノイズフロア推定と適応スケルチ（ベースライン減算）の数理

#### ① 記号一覧
| 記号 | 物理的・工学的意味 | 単位 / 基準比率 |
| :--- | :--- | :--- |
| $r(t)$ | 平滑化後の包絡線信号 | $[0.0, 1.0]$ |
| $r_{\text{floor}}$ | 移動・全体の低パーセンタイル（下位 25%）ノイズフロア | $[0.0, 1.0]$ |
| $r_{\text{peak}}$ | 上位 95% 信号ピーク強度 | $[0.0, 1.0]$ |
| $\Delta r$ | 動的ダイナミックレンジ ($r_{\text{peak}} - r_{\text{floor}}$) | $[0.0, 1.0]$ |
| $r_{\text{clean}}(t)$ | ベースライン減算・スケルチ後のクリーン包絡線 | $[0.0, 1.0]$ |

#### ② 日本語での読み解き
CW モールス通信の性質上、パルスのデューティ比は 30%〜40% 程度であり、観測区間の大半はスペース（無信号）です。したがって、包絡線振幅の下位 25% パーセンタイル値をとることで、周囲の環境雑音フロアを高精度に自己推定できます。有意な信号ピークが存在する場合、このフロア成分を差し引くことで、無信号区間における $750\,\text{Hz}$ 発振を完全に遮断（スケルチ）し、ウォーターフォールを漆黒に沈めます。

#### ③ 展開ステップと直感イメージ
$$V_{\text{th}} = r_{\text{floor}} + \max(0.15 \cdot \Delta r, 0.10 \cdot r_{\text{floor}})$$
$$r_{\text{clean}}(t) = \begin{cases} r(t) - r_{\text{floor}} & (r(t) > V_{\text{th}} \land \Delta r > 0.02) \\ 0.0 & (\text{otherwise}) \end{cases}$$

- **信号区間（ON）**: $r_{\text{clean}}(t) > 0$ となり、搬送波の純粋なパルス成分のみが $750\,\text{Hz}$ 正弦波を変調。
- **無信号区間（OFF）**: $r_{\text{clean}}(t) = 0$ となり、完全な無音（スペクトログラム上で $-120\,\text{dB}$ の漆黒）を実現。

---

### 6.4 軟化キーイング（Soft-keying Filter）による高調波抑圧

急激な矩形波状のオン・オフ（Hard Keying）は、キークリック雑音とともに無限の奇数次高調波スプリアス（$\sin((2k+1)\omega t)/(2k+1)$）を発生させます。
クリーン包絡線に対し、時定数 約 $1\,\text{ms}$ の移動平均フィルタ（軟化キーイング）を適用することで、パルスの立ち上がり・立ち下がりを滑らかにし、上部帯域に現れていた高調波歪みを抑圧してピュアな $750\,\text{Hz}$ 正弦波トーンを実現しました。

---

### 6.5 実生データ（raw.u8）の直接FFT解析とアンテナ北向き移設によるジオメトリ刷新

保全された `raw.u8`（133MB / 278.5秒）を 1 サンプル単位・全帯域（$\pm 120\,\text{kHz}$）FFT 探索で精密検証した結果、全区間における最大ピーク SNR は $1.7 \sim 2.6\,\text{dB}$（空間熱雑音と同等）であり、有意な衛星搬送波は到達していなかったことが判明しました。

#### ① なぜ未到達だったのか？（旧・東向きベランダの死角）
- 観測パス（2026-09-15 08:09:35 〜 08:14:35）のピーク方位は **西南西（WSW: 約 247.5°）** でした。
- マンション東向きベランダ（見通し方位 $0^\circ \sim 180^\circ$）から見ると、衛星は建物の真裏（西側）を通過しており、鉄筋コンクリート（RC）構造躯体による **$30 \sim 50\,\text{dB}$（1,000〜100,000倍）の物理的遮蔽減衰** を受けていました。

#### ② アンテナの「北向き」移設による受信ジオメトリの劇的改善
2026年9月15日、アンテナ設置場所を「東向き」から「北向き」へ移設しました。これにより、地上局の受信環境は極軌道衛星に対して極めて強力なジオメトリへと刷新されました：
...
1. **可視領域（FoV）の刷新**:
   - 見通し方位が **$270^\circ \sim 360^\circ/0^\circ \sim 90^\circ$（真西〜北西〜真北〜北東〜真東）** へと拡大。
   - これまで建物の真裏で全滅していた日本海・ユーラシア大陸側（西側〜北西）の通過パスが新たに受信可能エリアへ解放されました。
2. **極軌道衛星の降交点アプローチ（シベリア・北極海方面からの突入）を正面迎撃**:
   - 北から南へ飛行してくる極軌道衛星（Meteor-M や各種 CubeSat）を、地平線すれすれの北海道・サハリン・シベリア上空から真正面で捉えられるようになり、長時間の安定追跡と高SNR復調が可能になります。
